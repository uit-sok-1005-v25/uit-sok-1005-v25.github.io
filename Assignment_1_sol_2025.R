...
df_1 <- read_table2("https://www.nsstc.uah.edu/data/msu/v6.0/tlt/uahncdc_lt_6.0.txt") #Lower-Troposphere
df_2 <- read_table2("https://www.nsstc.uah.edu/data/msu/v6.0/tmt/uahncdc_mt_6.0.txt") #Mid-Troposphere
df_3 <- read_table2("https://www.nsstc.uah.edu/data/msu/v6.0/ttp/uahncdc_tp_6.0.txt") #Tropopause
df_4 <- read_table2("https://www.nsstc.uah.edu/data/msu/v6.0/tls/uahncdc_ls_6.0.txt") #Lower-Stratosphere
...
longdf$date <- as.Date(paste(longdf$Year, longdf$Mo, 1, sep="-"), "%Y-%m-%d") # uses the first of each month as day
longdf 
...
longdf %>%
  mutate(Year=year(Date)) %>% 
  filter(Year >= 1980) %>% 
pivot_wider(names_from = Location, values_from = Globe) %>% 
  mutate(Average =rowMeans(select(.,-c(Date,Year)))) %>% 
  pivot_longer(-c(Date,Year), names_to = "Location", values_to = "Averages") %>% 
  ggplot(aes(x=Date, y=Averages, col = Location))+geom_line()
